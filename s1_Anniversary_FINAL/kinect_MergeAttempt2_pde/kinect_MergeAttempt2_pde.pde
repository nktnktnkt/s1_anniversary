...
void setup() {
  ...
  //Pimage inits
  depthThreshold = createImage(1024, 424, HSB);
  depthPrevious = createImage(1024, 424, HSB);
  filteredDisplay = createImage(1024, 424, HSB);
...
void draw() {
  ...
  for (int i = 0; i < depthThreshold.width * depthThreshold.height; i++)
  {
    ...
    if (toggle == true) {
      ...
      if (dataIn[cS] > minDepth && dataIn[cS] < maxDepth)
      {
        
        dataMax[cS] = dataIn[cS] - minDepth;
        float c = dataMax[cS] * (theLfo % 20);
        carpetHue = map(lerp(carpetNum, carpetHue, 0.001), 1, 18, 1, 255);
