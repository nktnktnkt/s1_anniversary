...
void draw() {
  ...
  **************************************/
  ...
  {
     if (rawData[i] > minRange && rawData[i] < maxRange)
     {
       dataMax[i] = rawData[i] - minRange;
       float c = map(dataMax[i], 0, rangeDif, 0, 255);
       filteredDisplay.pixels[i] = color(c);
       if (depthThreshold.pixels[i] >= 0xf0f0f0){
          depthThreshold.pixels[i] -= 0x0f0f0f *1.0125;
       } else {
          depthThreshold.pixels[i] = 0xFFFFFF;
       }
     } else {
       if(depthThreshold.pixels[i] > 0x050505) {
         depthThreshold.pixels[i] -= 0x010101 *1.0125;
       } else {
